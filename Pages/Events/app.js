...
const imgUrls = [
  ...
  "/public/Events/pexels-pixabay-35888.jpg",
];
...
window.addEventListener("resize", () => {
  if (window.innerWidth < 900) {
    for (let i = 0; i < 5; i++) {
      document.querySelector(`.s-${i + 1}`).style.display = "none";
    }
  } else {
    for (let i = 0; i < 5; i++) {
      document.querySelector(`.s-${i + 1}`).style.display = "none";
    ...
    for (let i = 0; i < 5; i++) {
      document.querySelector(`.f-${i + 1}`).style.display = "none";
    }
    if (token) {
...
window.addEventListener("load", () => {
  ...
  fetch(dataUrls.openStatus, {
    ...
    .then(() => {
      ...
      let ctr = 0;
      for (let i = 0; i < 4; i++) {
        btnarr.push(document.querySelector(`#btn-${i + 1}`));
      }
      let regst = [];
      let opst = [];
        ...
        btnarr.forEach(function (elem, ptr) {
          elem.addEventListener("click", function () {
            ...
            if (regst[ptr] == true) {
              ...
              finalbtn.innerText = `Register`;
              if (opst[ptr] == true) {
                finalbtn.addEventListener("click", () => {
                  fetch(dataUrls.register, {
                    //Swap register with unregister for unregistering
                    method: "PUT",
                    headers: {
                      "Content-Type": "application/json",
                ...
                finalbtn.innerHTML = "<del>Register</del>";
              }
            }
          });
        });
...
fetch(dataUrls.getCode, {
  ...
  .then((data) => {
    code = data.code;
    console.log(code);
    document.querySelector(".ticket-profile_name").innerText = data.name;
    document.querySelector(".ticket-profile_username").innerText = data.email;
    document
      .querySelector(".ticket-profile_image")
      .setAttribute("src", `./../../public/avatars/${Number(code) + 1}.gif`);
    if (code === undefined) {
      document
        .querySelector(".ticket-profile_image")
        .setAttribute(
      ...
      document.querySelector(".ticket-profile_name").innerText = "UserName";
      document.querySelector(".ticket-profile_username").innerText = "Email";
    }
  });
  ...
  if (token) {
    for (let i = 0; i < 4; i++) {
      ...
      overlay[i].addEventListener("click", () => {
        document.querySelector(".c-137").innerText = eventdes[i];
...
window.onload = function () {
  ...
  css.type = "text/css";
  css.innerText = ".typewrite > .wrap { border-right: 3px solid lightgreen}";
